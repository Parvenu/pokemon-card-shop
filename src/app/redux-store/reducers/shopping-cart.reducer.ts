...
import { CartItem, FoilCount, ShoppingCart } from 'src/app/shared/models/shopping-cart.model'
import {produce} from "immer"
import { FOIL } from 'src/app/shared/models/card.model';
...
export const shoppingCartReducer = createReducer(
    ...
    on(ShoppingCartAction.removeCard, (shoppingCartState, { card, foil }) => {
    ...
    on(ShoppingCartAction.addCard, (shoppingCartState, { card, foil }) => {   
        const cartItems = shoppingCartState.items
        const cartItemIndex = cartItems.findIndex(ci => ci.card.id === card.id)
