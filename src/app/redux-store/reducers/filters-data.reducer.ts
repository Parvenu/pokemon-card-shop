...
export const initialState: {rarity: string[], types: string[], subtypes: string[]} = {rarity: [], types: [], subtypes: []};
...
export const filterTypesReducer = createReducer(
    ...
    on(FilterDataApiActions.retrievedRarityData, (state, {rarity}) => ({...state, rarity: state.rarity.concat(rarity)})),
    on(FilterDataApiActions.retrievedTypesData, (state, {types}) => ({...state, types: state.types.concat(types)})),
    on(FilterDataApiActions.retrievedSubtypesData, (state, {subtypes}) => ({...state, subtypes: state.subtypes.concat(subtypes)})),
)