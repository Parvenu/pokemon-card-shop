...
export class ShoppingCartComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.shoppingCartTotalPrice$ = this.store.select(shoppingCartState).pipe(
                map(sc => sc.items.reduce((acc, curr) => {
    ...
    public getItemFoils(foilCount: FoilCount): FOIL[] {
        return Object.keys(foilCount) as FOIL[]
    }

    public countItemTotalPrice(item: CartItem, foil?: FOIL): string {
        return (foil ? [foil] : Object.keys(item.foilCount) as FOIL[])
            .reduce((acc, foil) => {
                return acc += (item.foilCount[foil] * item.card.tcgplayer.prices[foil]!.low)
            }, 0).toFixed(2)
    }
