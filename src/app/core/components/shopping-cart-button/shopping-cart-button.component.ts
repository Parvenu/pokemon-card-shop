...
import { shoppingCartState } from 'src/app/redux-store/selectors/shopping-cart.selector';
...
export class ShoppingCartButtonComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.shoppingCartItemsCount$ = this.store.select(shoppingCartState).pipe(
                map(sc => sc.items.reduce((acc, cartItem) => {
                    return acc + Object.values(cartItem.foilCount).reduce((acc, foilCount) => {
